...
@Injectable()
export class EmployerBotService implements OnModuleInit {
  private bot: Telegraf;
  private readonly logger = new Logger(EmployerBotService.name);
  ...
  onModuleInit() {
    const token = process.env.TG_EMPLOYER_REGISTRATION_BOT_TOKEN;
    if (!token) {
      this.logger.error('TG_EMPLOYER_REGISTRATION_BOT_TOKEN не задан в .env');
      throw new Error('Токен Telegram-бота обязателен');
    }

    // Создаём бота ТОЛЬКО для отправки сообщений
    this.bot = new Telegraf(token);
    this.logger.log('EmployerBotService инициализирован (режим отправки)');
  }
